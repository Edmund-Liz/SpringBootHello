...
import com.springdemo.springboothello.Config.QiNiuYunConfig;
...
public class UploadImageServiceImp implements UploadImageService {
    ...
    public UploadImageServiceImp(QiNiuYunConfig qiNiuYunConfig) {
    ...
    public String getURL(MultipartFile file){
        ...
        if(!file.isEmpty()){
            String path =uploadQNImg(file);
            if(path.equals(result)){
                throw new RequestException("getURL","Pic Upload Fialed");
            }else{
                AppLogger.getLogger().info("Pic upload return:"+path);
                return path;
            }
        }else {
            throw new RequestException("getURL","Empty File");
        }
    }
    ...
    public String uploadQNImg(MultipartFile file) {
        ...
        try {
            ...
            if (!res.isOK()) {
                throw new RequestException("上传七牛出错：" ,res.toString());
            }
            // 解析上传成功的结果
